...
private _main = {
    ...
    if ((_executionType != "all")&&(_executionType != "selected")) exitwith {
        [[_executionType] call _getTargetsOfModule] call VTG_fnc_giveEquipExecution;
    };
    ...
    [_moduleAndTargets] call VTG_fnc_giveEquipExecution;
};

private _getTargetsAndNeededModules = {
    _units = param[0];
    
    _dataMap = call VTG_fnc_getAllmodulesFromStorage;
